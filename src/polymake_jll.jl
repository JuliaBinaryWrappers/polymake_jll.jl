...
if VERSION < v"1.3.0-rc4"
    ...
    error("Unable to import polymake_jll on Julia versions older than 1.3!")
end
...
if isdir(override_dir)
    ...
    function dev_jll()
        ...
        override_dir = joinpath(Pkg.devdir(), "polymake_jll", "override")
        # Copy the current artifact contents into that directory
        if !isdir(override_dir)
            cp(artifact"polymake", override_dir)
        end
        # Force recompilation of that package, just in case it wasn't dev'ed before
        touch(joinpath(Pkg.devdir(), "polymake_jll", "src", "polymake_jll.jl"))
        @info("polymake_ll dev'ed out to /depot/dev/polymake_jll with pre-populated override directory")
    end
end
...
artifacts = Pkg.Artifacts.load_artifacts_toml(artifacts_toml; pkg_uuid=UUID("7c209550-9012-526c-9264-55ba7a78ba2c"))
platforms = [Pkg.Artifacts.unpack_platform(e, "polymake", artifacts_toml) for e in artifacts["polymake"]]
...
if best_platform === nothing
    @debug("Unable to load polymake; unsupported platform $(triplet(platform_key_abi()))")
else
